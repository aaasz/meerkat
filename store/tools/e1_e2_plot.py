# Run like this: `python e1_e2_plot.py e1_e2.csv`.
...
def plot(filename: str, ylabel: str, line_datas: List[LineData]) -> None:
    ...
    for line_data in line_datas:
        ...
        linestyle = ('dashed'
                     if 'repl_scheme' in label and label['repl_scheme'] == 'lir'
                     else 'solid')
    ...
    ax.set_title('\n'.join(wrap(title, 40)))
    ax.set_xlabel('number of server cores')
...
def main(data_filename: str) -> None:
    ...
    ignore = {'num_server_threads', 'num_keys', 'num_client_machines',
        'num_clients_per_machine', 'num_threads_per_client',
        'follow_txn_avg_latency_success', 'tweet_txn_avg_latency_success'}
    outputs = {
        'num_transactions',
        ...
        if varied_columns:
            ...
            for (name, group) in grouped:
                if type(name) != list and type(name) != tuple:
                    name = [name]
                by_num_server_threads = group.groupby('num_server_threads')
            ...
            by_num_server_threads = df.groupby('num_server_threads')
            line_datas.append(LineData(
                label=dict(),
                mean=by_num_server_threads[output].mean(),
                std=by_num_server_threads[output].std()))

        filename = f'e1_e2_{output}.pdf'
