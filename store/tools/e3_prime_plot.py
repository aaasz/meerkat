        ...
        if varied_columns:
            ...
            for (name, group) in grouped:
                if type(name) != list and type(name) != tuple:
                    name = [name]
                by_zipf_coefficient = group.groupby('zipf_coefficient')
                line_datas.append(LineData(
                    label={k: v for (k, v) in zip(varied_columns, name)},
                    mean=by_zipf_coefficient[output].mean(),
                    std=by_zipf_coefficient[output].std()))
        else:
            by_zipf_coefficient = df.groupby('zipf_coefficient')
            line_datas.append(LineData(
                label=dict(),
                mean=by_zipf_coefficient[output].mean(),
                std=by_zipf_coefficient[output].std()))

        filename = f'e3_prime_{output}.pdf'
