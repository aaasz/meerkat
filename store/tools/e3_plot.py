# Run like this: `python e3_e4_e5_plot.py -e <e3|e4_e5> e3.csv`.
...
def main(data_filename: str, experiment_name: str) -> None:
    # Read in the data to a dataframe.
    df = pd.read_csv(data_filename)
    df['total_num_clients'] = (df['num_client_machines'] *
    ...
    # Create the charts.
    for output in ['throughput_success']:
        ...
        if varied_columns:
            ...
            for (name, group) in grouped:
                if type(name) != list and type(name) != tuple:
                    name = [name]
                by_num_server_threads = group.groupby('total_num_clients')
            ...
            by_num_server_threads = df.groupby('total_num_clients')
            line_datas.append(LineData(
                label=dict(),
                mean=by_num_server_threads[output].mean(),
                std=by_num_server_threads[output].std()))

        filename = f'{experiment_name}_{output}.pdf'
...
if __name__ == '__main__':
    ...
    main(args.data, args.experiment)
